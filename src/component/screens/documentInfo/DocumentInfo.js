...
function DocumentInfo(props) {
  ...
  useEffect(() => {
      ...
      if (obj && obj.token) {
          const {token} = obj;
          axios.get("http://localhost:4000/dts/fetchDocument/"+props.match.params.doc_id).then(res => {
              setDocument(res.data);

              axios.get("http://localhost:4000/dts/fetchActionReq/"+props.match.params.doc_id).then(res => {
